...
class TriggerPageProvider with ChangeNotifier {
  ...
  Future<Map<String, dynamic>> triggerMission() async {
    ...
    if (sizeController.text.isNotEmpty) {
      ...
      if (size == null) return {'success': false, 'message': '請輸入正確的數量'};
    }
    ...
    if (itemName.isNotEmpty && size != null) {
      destination["door"] = [
        {"id": "0", "orderList": [{"type": "normal", "name": itemName, "size": size}]},
        {"id": "1", "orderList": [{"type": "normal", "name": itemName, "size": size}]}
      ];
    }
    final payload = {
